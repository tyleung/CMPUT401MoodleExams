...
require_once($CFG->dirroot.'/local/rubricrepo_kboyle/kieran_form.php');
...
$PAGE->set_url($CFG->wwwroot.'/local/rubricrepo_kboyle/kieran.php');
...
if ($kieran_form->is_cancelled()) {
	...
	$check = $data->test1;
	redirect($CFG->wwwroot.'/local/rubricrepo_kboyle/view.php');//'/local/rubricrepo_kboyle/kieran.php?id='.$data->test1);
} else {
	echo $OUTPUT->header();
	$kieran_form->display();
	echo $OUTPUT->footer();
